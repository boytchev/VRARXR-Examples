...
function vaxInit( animateLoop, rendererOptions = {antialias:true} )
{
	animate = animateLoop;
...
function vaxInitVR( animateLoop, rendererOptions = {antialias:true} )
{
	vaxInit( animateLoop, rendererOptions );

	camera = new THREE.PerspectiveCamera( 60, 1, 0.01, 1000 );
	
	vrButton = VRButton.createButton( renderer ); // глобална променлива
...
function vaxInitAnaglyph( animateLoop )
{
	animate = animateLoop;
...
function vaxInitParallax( animateLoop, eyeSep = 1 )
{
	animate = animateLoop;
...
function vaxSceneInit(animateLoop, vax = vaxInit, vatInitParam = 0 )
{
	showStats = false;
...
export {
	...
	vaxInitAnaglyph, vaxInitParallax, effect, vaxInitVR };